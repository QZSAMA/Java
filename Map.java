...
public class Map implements Serializable{
    // 2D array
    private Creature[][] earth;
    int directions[]=new int[4];   //there are 4 directions, left, right,up and down
    int cells[]=new int[4];
    int plant[]=new int[30];  // 30*30 map will be created

    public int energy(int min) {  //generate the energy
        int consumption=min;
    ...
    public void growGrass() {
        for(int i=0; i<30; i++) {
            for(int j=0; j<30; j++) {
                if(earth[i][j].getNum()==0) plant[j]=1;
            }
        }
        for(int i=0; i<30; i++) {
            int rand=(int) (30*Math.random());
            for(int j=rand-1; j<-1; j--) {
                if(plant[j]==1) earth[i][j]=new Plant();
                break;
            }
    ...
    public Map() {this.earth=new Creature[30][30];}

    public void initializeEarth() {
        for(int i=0; i<30; i++) {
            for(int j=0; j<30; j++) {
                this.earth[i][j]=new Creature(0);
            }
        }
        for(int i=0; i<30; i++) {
            int rand=(int) (30*Math.random());
